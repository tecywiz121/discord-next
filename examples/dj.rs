    ...
    loop {
        ...
        match event {
            Event::MessageCreate(message) => {
                ...
                if first_word.eq_ignore_ascii_case("!dj") {
                    ...
                    if argument.eq_ignore_ascii_case("stop") {
                        ...
                        let output = if let Some((server_id, channel_id)) =
                            vchan
                        {
            ...
            Event::VoiceStateUpdate(server_id, _) => {
                ...
                if let Some(cur_channel) =
                    connection.voice(server_id).current_channel()
                {
                        ...
                        Some(server_id) => {
                                ...
                                .find(|srv| srv.id == server_id)
                            {
                                if srv
                                    ...
                                    .iter()
                                    .filter(|vs| {
                                        vs.channel_id == Some(cur_channel)
                                    })
                                    .count()
                                    <= 1
                                {
                                    connection
                                        .voice(Some(server_id))
                                        .disconnect();
                                }
                            }
                        }
