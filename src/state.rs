        ...
        match *event {
            ...
            Event::UserSettingsUpdate {
                ...
                if let Some(settings) = self.settings.as_mut() {
                    ...
                    opt_modify(
                        &mut settings.inline_embed_media,
                        inline_embed_media,
                    );
                    ...
                    opt_modify(
                        &mut settings.server_positions,
                        server_positions,
                    );
                    opt_modify(
                        &mut settings.show_current_game,
                        show_current_game,
                    );
                    opt_modify(&mut settings.status, status);
                    opt_modify(&mut settings.theme, theme);
                    opt_modify(
                        &mut settings.convert_emoticons,
                        convert_emoticons,
                    );
                    opt_modify(
                        &mut settings.friend_source_flags,
                        friend_source_flags,
                    );
                }
            }
            ...
            Event::VoiceStateUpdate(None, ref state) => {
                if let Some(channel) = state.channel_id {
                    ...
                    for call in self.calls.values_mut() {
                        call.voice_states
                            .retain(|u| u.user_id != state.user_id);
                    }
                }
            }
                    ...
                    match self.groups.entry(group.channel_id) {
                        ...
                        Entry::Occupied(mut e) => {
                            ...
                            if group.recipients.is_empty() {
                                ...
                                let recipients = ::std::mem::replace(
                                    &mut dest.recipients,
                                    Vec::new(),
                                );
...
fn update_presence(vec: &mut Vec<Presence>, presence: &Presence) {
    if presence.status == OnlineStatus::Offline {
        ...
        if let Some(srv_presence) =
            vec.iter_mut().find(|u| u.user_id == presence.user_id)
        {
