    ...
    pub fn decode_server(
        ...
        warn_json!(
            ...
            PublicChannel {
                ...
                nsfw: try!(opt(&mut value, "nsfw", |v| Ok(req!(v.as_bool()))))
                    .unwrap_or(false),
                parent_id: try!(opt(
                    &mut value,
                    "parent_id",
...
impl Invite {
    pub fn decode(value: Value) -> Result<Invite> {
        ...
        let server_id =
            try!(remove(&mut server, "id").and_then(ServerId::decode));
        let server_name =
            try!(remove(&mut server, "name").and_then(into_string));
        warn_field("Invite/guild", server);

        let mut channel =
            try!(remove(&mut value, "channel").and_then(into_map));
        let channel_type = try!(remove(&mut channel, "type").and_then(serde));
        let channel_id =
            try!(remove(&mut channel, "id").and_then(ChannelId::decode));
        let channel_name =
            try!(remove(&mut channel, "name").and_then(into_string));
...
impl RichInvite {
    pub fn decode(value: Value) -> Result<Self> {
        ...
        let server_splash_hash =
            try!(opt(&mut server, "splash_hash", into_string));
    ...
    pub fn decode(value: Value) -> Result<LiveServer> {
        ...
        warn_json!(
            ...
            LiveServer {
                ...
                member_count: req!(
                    try!(remove(&mut value, "member_count")).as_u64()
                ),
                ...
                afk_timeout: req!(
                    try!(remove(&mut value, "afk_timeout")).as_u64()
                ),
                afk_channel_id: try!(opt(
                    &mut value,
                    "afk_channel_id",
                    ChannelId::decode
                )),
                system_channel_id: try!(opt(
                    &mut value,
                    "system_channel_id",
                    ChannelId::decode
                )),
                channels: try!(decode_array(
                ...
                verification_level: try!(remove(
                    &mut value,
                    "verification_level"
                )
                .and_then(serde)),
                emojis: try!(remove(&mut value, "emojis")
                    .and_then(|v| decode_array(v, Emoji::decode))),
                features: try!(remove(&mut value, "features")
                ...
                explicit_content_filter: req!(try!(remove(
                    &mut value,
    ...
    fn decode(kind: String, value: Value) -> Result<Event> {
        ...
        if kind == "READY" {
            ...
            warn_json!(
                value,
                Event::Resumed {
                    trace: try!(remove(&mut value, "_trace").and_then(|v| {
            ...
            warn_json!(
                value,
                Event::CallUpdate {
                    channel_id: try!(remove(&mut value, "channel_id")
                        .and_then(ChannelId::decode)),
                    message_id: try!(remove(&mut value, "message_id")
            ...
            let channel_id =
                try!(remove(&mut value, "channel_id")
                    .and_then(ChannelId::decode));
            ...
            warn_json!(
                ...
                Event::TypingStart {
                    ...
                    timestamp: Utc.timestamp(
                        req!(try!(remove(&mut value, "timestamp")).as_i64()),
                        0
                    ),
                }
            )
            ...
            warn_json!(
                ...
                Event::MessageUpdate {
                    ...
                    mention_everyone: remove(&mut value, "mention_everyone")
                        .ok()
                        .and_then(|v| v.as_bool()),
                    mentions: try!(opt(&mut value, "mentions", |v| {
                        decode_array(v, User::decode)
                    })),
                    mention_roles: try!(opt(
                        &mut value,
                        "mention_roles",
                        |v| decode_array(v, RoleId::decode)
                    )),
                    attachments: try!(opt(&mut value, "attachments", |v| {
                        decode_array(v, Attachment::decode)
                    })),
                    embeds: try!(opt(&mut value, "embeds", |v| decode_array(
                        v, Ok
                    ))),
                }
            )
            ...
            warn_json!(
                ...
                Event::ServerMemberUpdate {
                    server_id: try!(remove(&mut value, "guild_id")
                        .and_then(ServerId::decode)),
                    roles: try!(decode_array(
                        try!(remove(&mut value, "roles")),
                        RoleId::decode
            ...
            warn_json!(
                ...
                Event::ServerSync {
                    ...
                    large: req!(try!(remove(&mut value, "large")).as_bool()),
                    members: try!(remove(&mut value, "members")
                        .and_then(|v| decode_array(v, Member::decode))),
                    presences: try!(decode_array(
                        try!(remove(&mut value, "presences")),
                        Presence::decode
            ...
            warn_json!(
                value,
                Event::ServerIntegrationsUpdate(try!(remove(
                    &mut value, "guild_id"
                )
                .and_then(ServerId::decode)),)
            )
            ...
            warn_json!(
                ...
                Event::ChannelPinsAck {
                    channel_id: try!(remove(&mut value, "channel_id")
                        .and_then(ChannelId::decode)),
                    timestamp: try!(remove(&mut value, "timestamp")
        ...
        if op == 2 {
            warn_json!(
                ...
                VoiceEvent::Handshake {
                    heartbeat_interval: req!(try!(remove(
                        &mut value,
                        "heartbeat_interval"
                    ))
                    .as_u64()),
                    modes: try!(decode_array(
                        try!(remove(&mut value, "modes")),
                        into_string
                    )),
                    port: req!(try!(remove(&mut value, "port")).as_u64())
                        as u16,
                    ssrc: req!(try!(remove(&mut value, "ssrc")).as_u64())
                        as u32,
            ...
            warn_json!(
                ...
                VoiceEvent::SpeakingUpdate {
                    ...
                    ssrc: req!(try!(remove(&mut value, "ssrc")).as_u64())
                        as u32,
                    speaking: req!(
                        try!(remove(&mut value, "speaking")).as_bool()
                    ),
                }
            )
        } else if op == 8 {
...
fn decode_shards(value: Value) -> Result<[u8; 2]> {
    ...
    Ok([
        req!(
            try!(array.get(0).ok_or(Error::Other("Expected shard number")))
                .as_u64()
        ) as u8,
        req!(try!(array
            .get(1)
