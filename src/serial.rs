...
fn i64_to_u64<'d, V: Visitor<'d>, E: Error>(
    v: V,
    n: i64,
) -> Result<V::Value, E> {
    if n >= 0 {
...
pub fn deserialize_discrim<'d, D: Deserializer<'d>>(
    d: D,
) -> Result<u16, D::Error> {
    ...
    ($typ:ident as $field:ident: $inner:path) => {
        ...
        impl<'d> ::serde::Deserialize<'d> for $typ {
            fn deserialize<D: ::serde::de::Deserializer<'d>>(
                ...
                <$inner as ::serde::de::Deserialize>::deserialize(d)
                    .map(|v| $typ { $field: v })
            }
        }
    };
...
pub mod named {
    ...
    pub fn serialize<T: NamedEnum, S: Serializer>(
        v: &T,
        s: S,
    ) -> Result<S::Ok, S::Error> {
        v.name().serialize(s)
    ...
    pub fn deserialize<'d, T: NamedEnum, D: Deserializer<'d>>(
        d: D,
    ) -> Result<T, D::Error> {
...
pub mod numeric {
    ...
    pub fn serialize<T: NumericEnum, S: Serializer>(
        v: &T,
        s: S,
    ) -> Result<S::Ok, S::Error> {
        v.num().serialize(s)
    ...
    pub fn deserialize<'d, T: NumericEnum, D: Deserializer<'d>>(
        d: D,
    ) -> Result<T, D::Error> {
