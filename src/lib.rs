...
fn tls_client() -> hyper::Client {
    let tls = hyper_native_tls::NativeTlsClient::new()
        .expect("Error initializing NativeTlsClient");
...
impl Discord {
    ...
    pub fn create_channel(
        ...
        let body = serde_json::to_string(&map)?;
        let response =
            request!(self, post(body), "/guilds/{}/channels", server);
    ...
    pub fn edit_channel<F: FnOnce(EditChannel) -> EditChannel>(
        ...
        match self.get_channel(channel)? {
            Channel::Private(_) => {
                return Err(Error::Other("Can not edit private channels"))
            }
    ...
    pub fn get_message(
        ...
        let response =
            request!(self, get, "/channels/{}/messages/{}", channel, message);
    ...
    pub fn send_message_ex<F: FnOnce(SendMessage) -> SendMessage>(
        ...
        let body = serde_json::to_string(&map)?;
        let response =
            request!(self, post(body), "/channels/{}/messages", channel);
        from_reader(response)
    }
    ...
    pub fn delete_messages(
        ...
        if ids.len() < 2 {
            return Err(Error::Other(
                "A minimum of 2 message ids must be supplied",
            ));
        } else if ids.len() > 100 {
            return Err(Error::Other(
                "A maximum of 100 message ids may be supplied",
            ));
        }
    ...
    pub fn send_file<R: ::std::io::Read>(
        ...
        let url = match hyper::Url::parse(&format!(
            api_concat!("/channels/{}/messages"),
            channel
        )) {
            Ok(url) => url,
        ...
        let mut request =
            hyper::client::Request::new(hyper::method::Method::Post, url)?;
        request
            .headers_mut()
    ...
    pub fn edit_emoji(
        ...
        let body = serde_json::to_string(&map)?;
        let response =
            request!(self, patch(body), "/guilds/{}/emojis/{}", server, emoji);
        from_reader(response)
    }
    ...
    pub fn create_invite(
        ...
        let body = serde_json::to_string(&map)?;
        let response =
            request!(self, post(body), "/channels/{}/invites", channel);
    ...
    pub fn get_member(&self, server: ServerId, user: UserId) -> Result<Member> {
        let response =
            request!(self, get, "/guilds/{}/members/{}", server, user);
        from_reader(response)
    }
    ...
    pub fn edit_role<F: FnOnce(EditRole) -> EditRole>(
        ...
        let body = serde_json::to_string(&map)?;
        let response =
            request!(self, patch(body), "/guilds/{}/roles/{}", server, role);
        Role::decode(serde_json::from_reader(response)?)
    }
    ...
    pub fn get_user_avatar(
        ...
        use std::io::Read;
        let mut response =
            retry(|| self.client.get(&self.get_user_avatar_url(user, avatar)))?;
        let mut vec = Vec::new();
...
fn from_reader<T: serde::de::DeserializeOwned, R: std::io::Read>(
    r: R,
) -> Result<T> {
...
pub fn get_unresolved_incidents() -> Result<Vec<Incident>> {
    let client = tls_client();
    let response =
        retry(|| client.get(status_concat!("/incidents/unresolved.json")))?;
    let mut json: Object = serde_json::from_reader(response)?;
...
pub fn get_active_maintenances() -> Result<Vec<Maintenance>> {
    ...
    match json.remove("scheduled_maintenances") {
        Some(scheduled_maintenances) => {
            decode_array(scheduled_maintenances, Maintenance::decode)
        }
        None => Ok(vec![]),
    }
}
...
impl ReceiverExt
    for websocket::client::Receiver<websocket::stream::WebSocketStream>
{
    fn recv_json<F, T>(&mut self, decode: F) -> Result<T>
    where
        F: FnOnce(serde_json::Value) -> Result<T>,
...
impl SenderExt
    for websocket::client::Sender<websocket::stream::WebSocketStream>
{
    fn send_json(&mut self, value: &serde_json::Value) -> Result<()> {
...
mod internal {
    pub enum Status {
        ...
        ChangeInterval(u64),
        ChangeSender(
            ::websocket::client::Sender<::websocket::stream::WebSocketStream>,
        ),
        Aborted,
    }
}
