...
pub fn open_ffmpeg_stream<P: AsRef<::std::ffi::OsStr>>(
    path: P,
...
pub fn open_ytdl_stream(url: &str) -> Result<Box<dyn AudioSource>> {
    ...
    let map = match json.as_object() {
        Some(map) => map,
        None => {
            return Err(Error::Other("youtube-dl output could not be read"))
        }
    };
            ...
            match channel.try_recv() {
                ...
                Ok(Status::Connect(info)) => {
                    connection = InternalConnection::new(info)
                        .map_err(|e| {
                            error!("Error connecting to voice: {:?}", e)
                        })
                        .ok();
                }
...
impl InternalConnection {
    fn new(info: ConnStartInfo) -> Result<InternalConnection> {
        ...
        let url = match ::websocket::client::request::Url::parse(&format!(
            "wss://{}",
            endpoint
        )) {
            Ok(url) => url,
            ...
            match receiver.recv_json(VoiceEvent::decode)? {
                ...
                other => {
                    ...
                    return Err(Error::Protocol(
                        "Unexpected message setting up voice",
                    ));
                }
            }
        }
...

        {
            ...
            let zero_index =
                bytes.iter().skip(4).position(|&x| x == 0).unwrap();
            ...
            let ws_thread = Some(
                ::std::thread::Builder::new()
                    ...
                    .spawn(move || {
                        ...
                        loop {
                            while let Ok(msg) =
                                receiver.recv_json(VoiceEvent::decode)
                            {
            ...
            let udp_thread = Some(
                ::std::thread::Builder::new()
                    ...
                    .spawn(move || {
                        ...
                        loop {
                            if let Ok((len, _)) =
                                udp_clone.recv_from(&mut buffer)
                            {
                                match tx2.send(RecvStatus::Udp(
                                    buffer[..len].to_vec(),
                                )) {
                                    Ok(()) => {}
                                    Err(_) => return,
    ...
    fn update(
            ...
            while let Ok(status) = self.receive_chan.try_recv() {
                match status {
                    ...
                    RecvStatus::Udp(packet) => {
                        ...
                        if let Ok(decrypted) = crypto::open(
                            &packet[HEADER_LEN..],
                            &nonce,
                            &self.encryption_key,
                        ) {
                            let channels =
                                opus::packet::get_nb_channels(&decrypted)?;
                            let len = self
                                .decoder_map
                                .entry((ssrc, channels))
                                .or_insert_with(|| {
                                    opus::Decoder::new(SAMPLE_RATE, channels)
                                        .unwrap()
                                })
        ...
        packet[HEADER_LEN..HEADER_LEN + crypted.len()]
            .clone_from_slice(&crypted);
