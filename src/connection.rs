...
impl Connection {
            ...
            GatewayEvent::InvalidateSession => {
                ...
                match receiver.recv_json(GatewayEvent::decode)? {
                    ...
                    other => {
                        debug!("Unexpected event: {:?}", other);
                        return Err(Error::Protocol(
                            "Expected Ready during handshake",
                        ));
                    }
                }
            }
    ...
    pub fn set_presence(
        ...
        let game = match game {
            ...
            Some(game) => {
                json! {{ "name": game.name, "type": GameType::Playing }}
            }
            None => json!(null),
        };
        let msg = json! {{
        ...
        loop {
            match self.receiver.recv_json(GatewayEvent::decode) {
                ...
                Ok(GatewayEvent::Dispatch(sequence, event)) => {
                    ...
                    let _ =
                        self.keepalive_channel.send(Status::Sequence(sequence));
                    #[cfg(feature = "voice")]
                    {
                        if let Event::VoiceStateUpdate(
                            server_id,
                            ref voice_state,
                        ) = event
                        {
                ...
                Ok(GatewayEvent::Heartbeat(sequence)) => {
                    ...
                    let _ =
                        self.keepalive_channel.send(Status::SendMessage(map));
                }
    ...
    fn reconnect(&mut self) -> Result<ReadyEvent> {
        ::sleep_ms(1000);
        self.keepalive_channel.send(Status::Aborted).expect(
            "Could not stop the keepalive thread, there will be a thread leak.",
        );
        trace!("Reconnecting...");
        // Make two attempts on the current known gateway URL
        for _ in 0..2 {
            if let Ok((conn, ready)) = Connection::__connect(
                &self.ws_url,
                &self.token,
                self.identify.clone(),
            ) {
        ...
        let url = ::Discord::from_token_raw(self.token.to_owned())
            .get_gateway_url()?;
        let (conn, ready) =
            Connection::__connect(&url, &self.token, self.identify.clone())?;
        ::std::mem::replace(self, conn).raw_shutdown();
        self.session_id = Some(ready.session_id.clone());
        Ok(ready)
    }
        ...
        loop {
            match receiver.recv_json(GatewayEvent::decode)? {
                ...
                GatewayEvent::Dispatch(seq, event) => {
                    ...
                    if let Event::Ready(ReadyEvent { ref session_id, .. }) =
                        event
                    {
                ...
                other => {
                    debug!("Unexpected event: {:?}", other);
                    return Err(Error::Protocol(
                        "Unexpected event during resume",
                    ));
                }
            }
        }
    ...
    fn inner_shutdown(&mut self) -> Result<()> {
        ...
        Sender::new(stream.by_ref(), true).send_message(
            &::websocket::message::Message::close_because(1000, ""),
        )?;
        stream.flush()?;
...
fn build_gateway_url(base: &str) -> Result<::websocket::client::request::Url> {
    ::websocket::client::request::Url::parse(&format!(
        "{}?v={}",
